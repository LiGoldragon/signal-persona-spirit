...
| Intent record queries support the agent-useful filters needed for intent work. | `RecordQuery` carries `TopicSelection` (`Any`, `Partial`, `Full`), optional `kind`, and description/provenance mode; partial topic filtering matches one or more requested topic memberships, and full filtering requires every requested topic. `RecordIdentifierQuery` carries exact or inclusive range selection by `RecordIdentifier`. |
